...
| 2 | Key delivery | **Committed module** `tools/app_secrets.py` (private repo; key-in-git tradeoff accepted). Write-scoped tokens baked by CI instead — see §7 |
...
> **Note (2026-07-08) — delivery split by risk class.** The Mouser key stays **committed**
> (this section, decision #2): free, rate-limited, replaceable, private repo → tradeoff
> accepted. Later **write-scoped** tokens do NOT follow it — the auto-updater token and the
> in-app git PAT are `None` in git and **baked at build time by CI** (`build-exe.yml`,
> `secrets.UPDATER_TOKEN` / `secrets.GIT_PAT`), because a repo-write token is a different
> risk class than a search key. See `tools/app_secrets.py`.
