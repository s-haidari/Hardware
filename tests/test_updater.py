...
def test_swap_script_retries_the_move():
    """Hardened swap: the move is retried in a loop (a briefly-locked exe — AV scan,
    slow process exit — should not leave the update un-applied). The original
    invariants (wait-for-pid, exact move, relaunch, self-delete) still hold."""
    script = U._swap_script(1234, Path(r"C:\a\KiCad Manager.exe.new"),
                            Path(r"C:\a\KiCad Manager.exe"))
    # still waits for the old process and self-deletes
    assert "PID eq 1234" in script
    assert script.endswith('del "%~f0"\r\n')
    # the move is now inside a bounded retry loop
    assert ":swaploop" in script
    assert "goto swaploop" in script
    assert 'move /Y "C:\\a\\KiCad Manager.exe.new" "C:\\a\\KiCad Manager.exe"' in script
    assert 'start "" "C:\\a\\KiCad Manager.exe"' in script
