...
def test_git_stage_commit_guard_passes_when_clean(tmp_path, monkeypatch):
    """A clean tree passes the corruption guard and proceeds to commit via nd_git.

    We stub nd_git.stage_all/commit so the test never shells out to git; the point
    is only that the guard did not abort and the wrapper delegated to nd_git.
    """
    import nd_git
    (tmp_path / "libs").mkdir()
    (tmp_path / "libs" / "MySymbols.kicad_sym").write_text(
    ...
    calls = []
    monkeypatch.setattr(nd_git, "stage_all",
                        lambda repo: calls.append(("stage_all", repo)) or nd_git.GitResult(ok=True))
    monkeypatch.setattr(nd_git, "commit",
                        lambda repo, message, paths=None: calls.append(("commit", message)) or (True, "abc1234"))
    cfg = {"RepoRoot": str(tmp_path)}
    log = DummyLog()
    assert L.git_stage_commit(cfg, log, message="ok") is True
    # Proceeded past the guard: staged then committed through nd_git.
    assert ("stage_all", str(tmp_path)) in calls
    assert ("commit", "ok") in calls
