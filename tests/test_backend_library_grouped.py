...
def test_scan_library_grouped_flags_and_grouping(tmp_path):
    ...
    # A footprint-only orphan has NO part number: the footprint stem is its human
    # label (name), but it must NOT be promoted into the identity's mpn field —
    # otherwise the detail view shows a fabricated 'Part Number' like 'FP_B'.
    assert c["name"] == "FP_B"          # the stem still drives the human label
    assert c["mpn"] is None             # ...but never the Part Number identity

    # A symbol-bearing part with no MPN property still resolves mpn to its symbol
    # name (a real canonical label for a bare passive), so the fix is scoped to
    # footprint-only orphans and does not blank out legitimate symbol rows.
    assert by_name["U1"]["mpn"] == "U1"
    assert by_name["U3"]["mpn"] == "U3"
...
def test_scan_library_grouped_has_real_mpn_from_strict_mpn(tmp_path):
    """The honest orderability signal (LM:2006): a symbol carrying a REAL
    manufacturer part number is has_real_mpn=True; a bare passive whose only
    identity is its symbol name is False and flagged not orderable — the same
    contract the BOM reads."""
    real = (
        '  (symbol "U_REG"\n'
        '    (property "Footprint" "MyFootprints:FP_A")\n'
        '    (property "Manufacturer Part Number" "TPS2121RUXR")\n'
        "    (pin 1)\n"
        "  )\n"
    )
    passive = _symbol("R_0603_10k", footprint="FP_B")   # no MPN property -> no real MPN
    cfg = _make_cfg(tmp_path, real + passive,
                    {"FP_A": "modelA.step", "FP_B": "modelA.step"}, ["modelA.step"])
    by_name = {r["name"]: r for r in L.scan_library_grouped(cfg)}

    assert by_name["U_REG"]["has_real_mpn"] is True
    assert by_name["U_REG"]["mpn"] == "TPS2121RUXR"
    assert L.part_display_names(by_name["U_REG"])["flag"] == ""

    assert by_name["R_0603_10k"]["has_real_mpn"] is False
    # 'mpn' still falls back to the symbol name for a human label, but the honest
    # signal marks the row not orderable and the shared flag surfaces it.
    assert by_name["R_0603_10k"]["mpn"] == "R_0603_10k"
    assert L.part_display_names(by_name["R_0603_10k"])["flag"] == L.NO_MPN_FLAG
