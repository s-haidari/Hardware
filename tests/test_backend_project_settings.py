...
def test_text_variables_remove_deletes_from_file(tmp_path):
    # M5: a UI "Remove" must actually delete the key from the file, not merely stop
    # re-writing it (the deep-merge would otherwise preserve it forever).
    pro = tmp_path / "B.kicad_pro"
    _write(pro, {"text_variables": {"KEEP": "k", "DROP": "d"}})
    m = PSM.ProjectSettingsManager()
    assert m.load_extended(pro)
    m.remove_text_variable("DROP")
    m.set_text_variable("NEW", "n")
    assert m.save_extended(pro)
    assert _read(pro)["text_variables"] == {"KEEP": "k", "NEW": "n"}


def test_text_variables_remove_last_then_empty(tmp_path):
    # Removing the only var writes an empty block (delete-the-last case), not a crash.
    pro = tmp_path / "B.kicad_pro"
    _write(pro, {"text_variables": {"ONLY": "x"}})
    m = PSM.ProjectSettingsManager()
    assert m.load_extended(pro)
    m.remove_text_variable("ONLY")
    assert m.save_extended(pro)
    assert "ONLY" not in _read(pro).get("text_variables", {})


def test_text_variables_readd_cancels_removal(tmp_path):
    # Re-adding a removed var cancels the pending deletion (set discards from removed set).
    pro = tmp_path / "B.kicad_pro"
    _write(pro, {"text_variables": {"REV": "A"}})
    m = PSM.ProjectSettingsManager()
    assert m.load_extended(pro)
    m.remove_text_variable("REV")
    m.set_text_variable("REV", "B")
    assert m.save_extended(pro)
    assert _read(pro)["text_variables"]["REV"] == "B"


def test_text_variables_removal_reset_on_reload(tmp_path):
    # M5 review MED: a reload is a full re-read — a stale pending removal must NOT re-delete a
    # key that has since reappeared in the file (data-loss footgun on a reused manager).
    pro = tmp_path / "B.kicad_pro"
    _write(pro, {"text_variables": {"REV": "A", "KEEP": "k"}})
    m = PSM.ProjectSettingsManager()
    assert m.load_extended(pro)
    m.remove_text_variable("REV")
    assert m.save_extended(pro)
    assert "REV" not in _read(pro)["text_variables"]
    _write(pro, {"text_variables": {"KEEP": "k", "REV": "Z"}})   # another tool reintroduces REV
    assert m.load_extended(pro)                                   # reload clears _removed_text_vars
    m.set_text_variable("KEEP", "k2")                             # an unrelated edit
    assert m.save_extended(pro)
    assert _read(pro)["text_variables"].get("REV") == "Z"        # NOT silently re-deleted


def test_text_variables_nonstring_value_coerced(tmp_path):
    # M5 review MED: a malformed non-string value must be coerced at load, not reach a
    # QLineEdit raw (which would abort the whole Editor panel build).
    pro = tmp_path / "B.kicad_pro"
    _write(pro, {"text_variables": {"NUM": 5, "FLAG": True}})
    m = PSM.ProjectSettingsManager()
    assert m.load_extended(pro)
    assert m.text_variables == {"NUM": "5", "FLAG": "True"}


def test_text_variables_phantom_removal_no_manufacture(tmp_path):
    # M5 review LOW: removing a var that was never in the file must NOT manufacture an empty
    # text_variables:{} block into a file that had none (no-manufacture-by-default contract).
    pro = tmp_path / "B.kicad_pro"
    _write(pro, {"meta": {"version": 1}})
    m = PSM.ProjectSettingsManager()
    assert m.load_extended(pro)
    m.remove_text_variable("GHOST")
    assert m.save_extended(pro)
