...
class PartIdentityTests(unittest.TestCase):
    ...
    def test_grouped_scan_carries_identity(self):
        ...
        # A symbol-bearing part always resolves to a canonical name (real MPN or,
        # for a bare passive, its symbol name). A footprint-only orphan has NO part
        # number — its mpn must stay None, never the promoted footprint stem.
        self.assertTrue(all(r["mpn"] for r in rows if r["has_symbol"]),
                        "a symbol-bearing row must carry a canonical name")
        self.assertTrue(all(r["mpn"] is None for r in rows if not r["has_symbol"]),
                        "a footprint-only orphan must not surface a fake part number")
    ...
    def test_providers_from_config(self):
        ...
        with mock.patch.dict(os.environ, {}, clear=False):
            ...
            with mock.patch.object(LM.app_secrets, "MOUSER_API_KEY_DEFAULT", ""), \
                    mock.patch.object(LM, "resolve_digikey_creds", lambda cfg=None: (None, None)):
                # No Mouser key AND LCSC off AND no DigiKey creds -> genuinely no
                # providers. DigiKey is isolated from any real config.json creds
                # (SRC-04 added config.json as a DigiKey user path).
                self.assertIsNone(LM.providers_from_config({"LcscSourcing": "0"}))
                self.assertIsNone(LM.providers_from_config(
                    {"MouserApiKey": "k", "LcscSourcing": "0"}))                        # config key ignored
                # LCSC (key-free) is the default fallback -> live even with no Mouser key.
                with mock.patch.object(LM, "read_setting", lambda key, default=None, **k: default):
                    self.assertIsNotNone(LM.providers_from_config({}))
            with mock.patch.object(LM.app_secrets, "MOUSER_API_KEY_DEFAULT", "baked"):
                self.assertIsNotNone(LM.providers_from_config({"LcscSourcing": "0"}))   # baked key -> live
