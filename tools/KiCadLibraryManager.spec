...
# UI assets resolved at runtime via theme._fonts_dir(). (The icon set is inline SVG
# strings in ui.icons — no bundled icon files; the old Lucide SVGs are retired.)
datas = []
...
a = Analysis(
    ...
    hiddenimports=[
        'PyQt5.QtSvg',               # inline-SVG icon rendering (ui.widgets.svg_icon)
        'app_secrets',               # baked Mouser key (SP1)
        'app_build',                 # CI-stamped version/repo (updater)
        'nd_updater',                # in-app GitHub-release auto-updater
        'ui.shell',                  # the styled shell (entry — python -m ui)
        'nd_project_health',         # ui.features.projects (function-level import)
        'nd_kicad_checks',           # ui.features.projects ERC/DRC (function-level import)
        'kicad_paths',               # ui.features.projects/settings (function-level import)
        'nd_git',                    # ui.features.git (function-level import)
        'ui.features',
        'ui.features.bench',
        'ui.features.library',
